...
func ExtractTraceStatistics(trace *TraceData) (*TraceStatistics, map[string][]byte) {
	...
	for _, span := range trace.Spans {
		...
		for _, a := range span.Attributes {
			switch a.Key {
			case "from":
				from = a.Value // full name (nodeID:port) for edge messages, "signal" for signals
			case "to":
				to = a.Value // full name (nodeID:port)
			case "port":
				port = a.Value
			}
		}
...
func ApplyTraceStatToEdge(edgeMap map[string]interface{}, ts *TraceStatistics) {
	...
	edgeKey := from + "|" + to
	latency, hasLatency := ts.Latencies[edgeKey]
	...
	seq := getIndexByValue(ts.Sequence, edgeKey)
