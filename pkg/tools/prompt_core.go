...
- **Time:** none. There is no ` + "`now()`" + ` and no date function of any kind — an expression can only reshape what arrived. To stamp a message with a time, compute it in a ` + "`js_eval`" + ` node (` + "`new Date().toISOString()`" + `) and map that field on the edge.
