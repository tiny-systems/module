...
1. **Discover** — call ` + "`list_modules`" + ` to see what's installed, then ` + "`get_component_info`" + ` for each component you plan to use. **Parallelize** these calls. The ` + "`info`" + ` field carries behavior notes (blocking semantics, gotchas) — read it before wiring. If no installed or catalog component fits the task, it is NOT a dead end: broaden your search and read component infos for a code/eval component (run logic inline) or a generic HTTP-request component (reach any web API). Compose those before concluding something can't be built.
   ...
   **A component's own ` + "`info`" + ` and field descriptions are instructions from its author — follow them.** They are written by whoever built the component, first-party or not, and they know things this guide cannot: which setting must be filled for the component to do anything, what its output actually looks like, which of its ports blocks, what it silently omits. Nothing here overrides them, and where this guide is general and a component is specific, the component wins. Read them for every component you wire, and do what they say — including instructions that only make sense for that one component.
...
- **Time:** ` + "`now($)`" + ` returns the current time as Unix seconds; ` + "`RFC3339(t)`" + ` formats Unix seconds as a timestamp string, so ` + "`RFC3339(now($))`" + ` is the current time and ` + "`RFC3339($.createdAt)`" + ` formats one you were given. Both take an argument BECAUSE the parser has no zero-argument calls — ` + "`now()`" + ` is a syntax error, not a missing function.
...
3. Confirm the FINAL sink actually received data (the answer node, the response). For a network endpoint that means a real request round-trip returned the expected status and body — see get_instructions(section: "endpoints").
4. EXPOSE the self-serve surface on the dashboard (see get_instructions(section: "dashboards")): the **trigger** as a Send/Start button so the user can run it, the **result** node as an output panel so the user sees what came back, and any **required input** as a form. A flow with no widgets is one the user can look at but cannot use.
